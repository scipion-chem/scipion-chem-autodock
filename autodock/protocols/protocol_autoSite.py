...
class ProtChemAutoSite(ProtChemAutodockBase):
    """Perform a pocket find experiment with AutoSite. See the help at
       ...
       User IA Manual: AutoSite Protocol

The AutoSite protocol identifies potential ligand binding pockets on a 
macromolecular structure by detecting regions of favorable interaction using 
AutoDock grid maps. It plays a central role in early-stage structure-based drug 
design, especially when the precise location of the binding site is unknown or 
requires computational validation. The protocol operates by analyzing one or 
more affinity maps generated by AutoGrid or similar tools, typically derived 
from a receptor file processed in PDBQT format.

To initiate the protocol, the user must provide a `.maps.fld` file, which 
contains the 3D affinity grids for the receptor. The protocol also accepts a 
receptor PDBQT file to allow visualization and interpretation of the predicted 
sites in the structural context. Optionally, a ligand file may be supplied to 
help rank the predicted pockets or to evaluate whether any predicted site 
overlaps with known ligand positions, but the ligand is not mandatory.

The core of the protocol involves identifying clusters of favorable interaction 
energy across the grid. These clusters are interpreted as candidate binding 
pockets. Several parameters control how this clustering is performed. The 
threshold parameter defines the energy cutoff used to identify regions likely 
to host ligand atoms. Lowering the threshold typically increases sensitivity by 
detecting weaker interaction points, whereas raising it focuses the detection 
on high-affinity areas. The minimum volume parameter determines the smallest 
volume a region must occupy to be considered a valid site. This helps to avoid 
noise or spurious small cavities that would not accommodate a drug-like 
molecule.

The user can also control the maximum number of pockets that will be reported. 
This is useful for filtering the output when many potential sites are detected 
and only the top-ranked ones are of interest. When a reference ligand is 
provided, an overlap criterion can be applied so that only predicted sites 
within a certain distance from the ligand are retained. This distance cutoff 
ensures that irrelevant pockets are discarded in favor of those supported by 
prior experimental or modeled information.

The output of the protocol consists of a set of predicted binding pockets, 
which can be visualized directly in Scipion using molecular viewers. These 
pockets can be further processed by other protocols such as pharmacophore 
extraction, docking, or site-directed mutagenesis analysis. In addition, the 
user may inspect the log files to examine scoring and clustering statistics that 
justify the selection of each pocket. By combining receptor maps, energy 
thresholding, spatial filtering, and optional ligand proximity constraints, this 
protocol provides a powerful and automated way to explore the interaction 
landscape of a receptor."""
    _label = 'AutoSite'
