...
class ProtChemAutodockScore(ProtChemAutodockBase):
    ...
    User IA Manual: AutoDock Score Protocol

    Overview:
    The AutoDock Score protocol evaluates ligand?receptor complexes using AutoDock's scoring function.
    It is intended for rescoring poses previously generated by docking tools or molecular simulations.
    The protocol parses each complex, applies AutoDock's energy scoring, and stores the results for
    downstream analysis or filtering.

    This protocol assumes that the receptor and ligands are already properly prepared, including correct
    protonation, structural formatting, and charge assignment, and are provided in the PDBQT format.

    Input Parameters:
    The protocol requires an input complex set, typically a ChemStructSet where each item represents a
    ligand bound to a receptor or positioned within the binding site context. These must be in PDBQT format.

    A reference receptor must also be specified. This receptor should be the fixed macromolecule used
    during the docking phase and must also be provided in PDBQT format. It should not include ligands,
    solvent molecules, or other extraneous atoms.

    Optionally, a grid box can be defined to restrict the scoring region. This box is specified by its
    center coordinates (X, Y, Z), the number of points along each axis, and the grid spacing. If no box
    is defined, the scoring will be computed over the entire receptor structure.

    Execution:
    Once configured, the protocol scores each complex using the AutoDock scoring function. Key terms such
    as binding free energy, intermolecular energy, torsional energy, and internal energy are calculated.
    These values are added to the structure annotations and saved in the output set.

    Any structures that cannot be processed (due to format errors or missing atoms) are skipped and reported
    in the log, allowing the protocol to continue with the remaining entries.

    Outputs:
    The output is a new ChemStructSet containing all successfully scored complexes. Each entry includes the
    computed AutoDock score and related energy terms in its metadata. This dataset can be explored
    interactively in Scipion using the built-in viewer or exported for further analysis.

    A summary log file is also provided, which details the scoring run, including the number of processed
    structures, time taken, and any encountered issues.

    Usage Notes:
    This protocol is ideal for rescoring or re-ranking ligand poses from previous docking runs, especially
    when comparing relative binding affinities across different compounds. It is essential that all input
    files remain in PDBQT format throughout the workflow to ensure proper parsing and scoring.

    Care should be taken when defining the grid box, as an improperly configured region could lead to
    inaccurate scoring results. The box should enclose the actual binding site fully.

    The scoring procedure is purely analytical and does not modify input structures. It is deterministic
    and reproducible under consistent input conditions.

    Compatibility:
    The protocol is designed to work seamlessly with other Scipion Chem modules that output PDBQT structures.
    It can be inserted into docking or screening workflows to provide standardized rescoring using the
    AutoDock energy model.
    """
